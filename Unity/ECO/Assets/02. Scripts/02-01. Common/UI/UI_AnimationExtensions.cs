...
public static class UI_AnimationExtensions
{
    ...
    {
        graphic.DOFade(targetFadeValue, fadeTime).SetEase(Ease.OutQuad).OnComplete(() =>
        {
            foreach (var action in onCompleteActions)
            {
                action?.Invoke();
    ...
    {
        canvasGroup.DOFade(targetFadeValue, fadeTime).SetEase(Ease.OutQuad).OnComplete(() =>
        {
            foreach (var action in onCompleteActions)
            {
                action?.Invoke();
    ...
    public static void PopIn(this RectTransform target, float time, params System.Action[] onCompleteActions)
    {
        ...
        target.DOScale(Vector3.one, time).SetEase(Ease.OutBack).OnComplete(() =>
        {
            foreach (var action in onCompleteActions)
            {
                action?.Invoke();
    ...
    {
        target.DOScale(Vector3.zero, time).SetEase(Ease.InBack).OnComplete(() =>
        {
            foreach (var action in onCompleteActions)
            {
                action?.Invoke();
    ...
    {
        target.DOPunchScale(new Vector3(0.1f, 0.1f, 0.1f), duration, 1, 1f).OnComplete(() =>
        {
            foreach (var action in onCompleteActions)
            {
                action?.Invoke();
    ...
    {
        target.DOShakeAnchorPos(duration, new Vector2(10f, 0f), 10, 90f, false, true).OnComplete(() =>
        {
            foreach (var action in onCompleteActions)
            {
                action?.Invoke();
